...
\begin{center}
    ...
    Luego realizamos una sustitucion trigonometrica, $ u = \tan(x)$.
    \[\displaystyle\ \int_{0}^{\infty}\frac{\sqrt[3]{u}}{{(u+1)}^2}dx\]
    para volver esta integral a una forma mas conveniente, podemos realizar la sustitucion $v = u^{\frac{1}{3}}$,
    lo que nos dejaria con: $ 3v^{3}dv = u^{\frac{1}{3}}du$ y nuestra integral nueva seria:
    \[\displaystyle\ \int_{0}^{\infty}\frac{3v^{3}}{(v^{3}+1)^2}dv = \displaystyle\ \int_{0}^{\infty}\frac{3v^{2}v}{{(v^{3}+1)}^2}dv\]
    Luego utilizamos integracion por partes:
    ...
    \[
        \left[
            \begin{aligned}
                u  & = x  \\
                du & = dx \\
                dv & =  \frac{3x^2}{{(1+x^3)}^2}  \\
                v  & = \frac{-1}{(1+x^3)}
            \end{aligned}
            \right]
    \]

    Luego, la integracion por partes nos quedaria como: 
    \[\displaystyle\ \int_{0}^{\infty}\frac{3x^{3}}{{(x^{3}+1)}^2}dx = \frac{-x}{(1+x^3)} \Big|_0^{\infty} + \int_{0}^{\infty}\frac{dx}{(1+x^3)} \]
    Ahora solo nos queda resolver la integral a al lado derecho, para esto, utilizaremos un cambio de variable $u = \frac{1}{x}$, lo que nos dejaria con:

    \[-\int_{\infty}^{0} \frac{dx}{1+x^3} = \int_{0}^{\infty} \frac{dx}{1+x^3}\]
...
\begin{center}
    ...
    \[\displaystyle\    \frac{1}{7\sqrt{2}} \leq \int_{0}^{1}\frac{{x}^6}{\sqrtsign{1+{x}^2}}dx \leq \frac{1}{7}\]
